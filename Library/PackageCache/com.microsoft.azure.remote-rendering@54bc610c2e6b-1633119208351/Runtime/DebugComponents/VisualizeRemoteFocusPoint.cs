...
public class VisualizeRemoteFocusPoint : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        _validResult = FocusPointResult.Invalid;

        IntPtr ptr = IntPtr.Zero;
        // Obtain the focus point using the same coordinate system that is used as user coordinate system.
#if WINDOWS_UWP
        if (RemoteManagerUnity.CurrentSession.GraphicsBinding is GraphicsBindingWmrD3d11)
        {
...
#endif
        _validResult = RemoteManagerUnity.CurrentSession.GraphicsBinding.GetRemoteFocusPoint(ptr, out position, out normal, out velocity);
