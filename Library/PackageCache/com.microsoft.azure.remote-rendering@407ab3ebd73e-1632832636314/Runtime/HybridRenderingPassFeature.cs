...
public class HybridRenderingPassFeature : ScriptableRendererFeature
{
    internal class HolographicRemotingCallbackPass : ScriptableRenderPass
    {
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            if (!_renderRemoteFrameCallback.Equals(IntPtr.Zero))
            {
                ...
                if (_urpTemporaryColorBuffer.HasValue)
                {
    ...
    {
        if (!RemoteManagerUnity.IsInitialized || !RemoteManagerUnity.IsConnected)
        {
            return;
        }

        bool isUsingSimulation = RemoteManagerUnity.CurrentSession.GraphicsBinding.Api == GraphicsApiType.SimD3D11;
